...
class TransFuse_S(nn.Module):
    def __init__(self, num_classes=1, drop_rate=0.2, normal_init=True, pretrained=False):
        ...
        if pretrained:
            self.resnet.load_state_dict(torch.load('/home/zhangyanhua/Code_python/model_pretrained/resnet34-333f7ec4.pth'))
    ...
    def forward(self, imgs, labels=None):
        ...
        # 有的数据集是 grey image
        if imgs.size()[1] == 1:
            imgs = imgs.repeat(1,3,1,1) # 变成 3 Channel 的输入。在这里啊！！！把 single channel 的 slice 扩展为 3 通道的。
        ...
        output = F.interpolate(self.final_2(x_c_2), scale_factor=4, mode='bilinear', align_corners=True)
...
class TransFuse_L(nn.Module):
    def __init__(self, num_classes=1, drop_rate=0.2, normal_init=True, pretrained=False):
        ...
        if pretrained:
            self.resnet.load_state_dict(torch.load('/home/zhangyanhua/Code_python/model_pretrained/resnet50-19c8e357.pth'))
        self.resnet.fc = nn.Identity()
        self.resnet.layer4 = nn.Identity()
