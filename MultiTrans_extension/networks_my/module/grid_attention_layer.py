...
# 这里要加入 final_out_channels=None
class _GridAttentionBlockND(nn.Module):
    def __init__(self, in_channels, gating_channels, inter_channels=None, final_out_channels=None, dimension=3, mode='concatenation',
                 sub_sample_factor=(2,2,2)):
        super(_GridAttentionBlockND, self).__init__()

        assert dimension in [2, 3]
        assert mode in ['concatenation', 'concatenation_debug', 'concatenation_residual', 'concatenation_MultiTrans']
        ...
        self.W = nn.Sequential(
            conv_nd(in_channels=self.in_channels, out_channels=self.final_out_channels, kernel_size=1, stride=1, padding=0),
            bn(self.final_out_channels),
        )
        ...
        # 1x1 kernel 相当于 linear projection
        ...
        elif mode == 'concatenation_MultiTrans':
            self.operation_function = self._concatenation_MultiTrans
    ...
    def _concatenation_MultiTrans(self, x, g):
        input_size = x.size()
        ...
        # 1x1 conv 进行 channel 调整。然后再进行上采样
        # B, C, H, W --> B, C', H, W
        ...
        # 再输入 sigmoid 激活函数
        # 先将 channel 映射到 1
        # sigm_psi_f = F.sigmoid(self.psi(f))  # "nn.functional.sigmoid is deprecated. Use torch.sigmoid instead."
        sigm_psi_f = torch.sigmoid(self.psi(f))

        # upsample the attentions and multiply
        # 再上采样到原输入 x 的大小
        # sigm_psi_f = F.upsample(sigm_psi_f, size=input_size[2:], mode=self.upsample_mode)  # nn.functional.upsample is deprecated. Use nn.functional.interpolate instead.
        sigm_psi_f = F.interpolate(sigm_psi_f, size=input_size[2:], mode=self.upsample_mode)

        # 因为 sigm_psi_f 的 channel 为 1 。这里的 expand_as() 是进行 channel 的调整扩充，扩充到与 x 的 channel 相同。
        # 然后与 x 进行 pixel-wise 的 multiplication
        y = sigm_psi_f.expand_as(x) * x

        # 这里 filter 后的 local feature 与 global feature 融合
        fuse = y + g
...
class GridAttentionBlock2D(_GridAttentionBlockND):
    def __init__(self, in_channels, gating_channels, inter_channels=None, final_out_channels=None, mode='concatenation',
